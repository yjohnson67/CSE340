...
ALTER TYPE public.account_type OWNER TO cse340render_user;
...
ADD CONSTRAINT kf_classification FOREIGN KEY (classification_id) REFERENCES public.classification (classification_id) MATCH SIMPLE ON UPDATE CASCADE ON DELETE NO ACTION;
...
SELECT inventory.inv_make,
  inventory.inv_model,
  classification.classification_name --Where are all of these things located?--
From inventory --What resources are we going to be adjusting?
  INNER JOIN classification ON inventory.classification_id = classification.classification_id --Where are we making these changes?
